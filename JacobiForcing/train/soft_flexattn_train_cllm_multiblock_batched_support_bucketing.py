...
class LengthBucketBatchSampler(Sampler[list[int]]):
    """
    Bucket by rounded-up length so Lmax takes only a small set of values.
    Each yielded item is a list of indices (a batch).
    """
    def __init__(
        self,
        lengths: list[int],
        batch_size: int,
        bucket_size: int = 256,   # round lengths to multiples of this
        shuffle: bool = True,
        drop_last: bool = True,
        seed: int = 0,
    ):
        ...
        # build buckets: key is rounded-up length
        buckets = {}
        for i, L in enumerate(lengths):
            key = ((int(L) + self.bucket_size - 1) // self.bucket_size) * self.bucket_size
            buckets.setdefault(key, []).append(i)
        self._bucket_keys = sorted(buckets.keys())
        self._buckets = buckets

        # precompute batch count for __len__
        total = 0
    ...
    def __iter__(self):
        ...
        # optionally shuffle bucket order, and items within each bucket
        bucket_keys = list(self._bucket_keys)
        if self.shuffle:
        ...
        for key in bucket_keys:
            ...
            # emit batches within this bucket
            bs = self.batch_size
            n = len(idxs)
            end = n - (n % bs) if self.drop_last else n
            for s in range(0, end, bs):
                batch = idxs[s:s + bs]
                if len(batch) == bs or (not self.drop_last and len(batch) > 0):
                    yield batch
...
def train():
    ...
    # Precompute lengths for bucketing (len of complete_training_sequence_ids)
    # This uses the underlying HF dataset, not the cached preprocessing dicts.
    raw_ds = trajectory_dataset  # datasets.Dataset
    lengths = []
    if accelerator.is_local_main_process:
        rank0_print(training_args.local_rank, "Precomputing sequence lengths for bucketing...")
    for i in tqdm(range(len(raw_ds)), desc="Computing lengths", disable=not accelerator.is_local_main_process):
        # list length only
        lengths.append(len(raw_ds[i]["complete_training_sequence_ids"]))
    
    if accelerator.is_local_main_process:
        rank0_print(training_args.local_rank, f"Computed lengths for {len(lengths)} samples")
    
    # Ensure all processes have computed lengths before proceeding
    ...
    # Bucketed batching so Lmax repeats far less often --> compile & cache masks per bucket.
    batch_sampler = LengthBucketBatchSampler(
        lengths=lengths,
        batch_size=per_device_bs,
        bucket_size=256,     # you can tune: 128/256/512
        shuffle=True,
        drop_last=True,      # keep B constant for compiled mask
        seed=int(training_args.seed) if training_args.seed is not None else 0,
    )
